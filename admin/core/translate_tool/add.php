...
if($action == 'execute'){
    ...
    $upload_voice_ci = upload_file('voice_ci',$file_path);
    if($upload_voice_ci['success']){
        //upload thành công
        $file_name = $upload_voice_ci['file_name'];
        $myform->add('dic_voice_ci','file_name',0,1,'');
    }else{
        ...
        if(file_exists($output_recorder)){
            ...
            if(rename($output_recorder,$file_path)){
                ...
                $bg_errorMsg .= '&bull;'.$upload_voice_ci['error'];
    ...
    $upload_voice_ci_woman = upload_file('voice_ci_woman',$file_path);
    if($upload_voice_ci_woman['success']){
        //upload thành công
        $file_name_wm = $upload_voice_ci_woman['file_name'];
        $myform->add('dic_voice_ci_woman','file_name_wm',0,1,'');
    }else{
        //không thành công, tìm file trong thư mục ghi âm
        ...
        if(file_exists($output_recorder)){
            ...
            if(rename($output_recorder,$file_path)){
                ...
                $bg_errorMsg .= '&bull;'.$upload_voice_ci_woman['error'];
                $bg_errorMsg .= '&bull;Không thể di chuyển file ghi âm';
            }
        }else{
            $bg_errorMsg .= '&bull;'.$upload_voice_ci_woman['error'];
...
$html_page .= $form->textarea(array(
    ...
    'value'=>getValue('translate_ci','str','POST','')
));
